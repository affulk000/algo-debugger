...
/**
 * Custom string input for LongestSubstring.
 *
 * @param {Function} onApply  Called with (s: string) on valid submit
...
export default function CustomInput({ onApply, T }) {
  ...
  const apply = () => {
    ...
    if (trimmed.length > 26) {
      setError("Max 26 characters for readability.");
      return;
    }
    setError("");
    onApply(trimmed);
