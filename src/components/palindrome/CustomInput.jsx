...
export default function CustomInput({ onApply, T }) {
  ...
  const apply = () => {
    const trimmed = val;
    if (trimmed.length === 0) { setError("Please enter a string."); return; }
    if (trimmed.length > 60)  { setError("Max 60 characters for readability."); return; }
    setError("");
    onApply(trimmed);
  };
  ...
  return (
    <div style={{
      ...
      <input name="palindrome-s" value={val} onChange={e => setVal(e.target.value)}
