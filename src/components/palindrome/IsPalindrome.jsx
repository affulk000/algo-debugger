...
export default function IsPalindrome({ T }) {
  ...
  const { meta } = useAlgoMeta("palindrome");
  ...
  const { steps, loading, error } = useBackendSteps("palindrome", { s: activeStr }, [presetIdx, custom], { enabled: presets.length > 0 });

  const onComplete = useAlgoRun("palindrome", () => JSON.stringify({ s: activeStr }));
  ...
  return (
    <div>
      <PresetBar
        presets={presets}
        getLabel={p => p.label}
        getSublabel={p => `"${p.s.slice(0, 20)}${p.s.length > 20 ? "…" : ""}"  →  ${p.answer}`}
