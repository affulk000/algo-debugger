...
export default function CustomInput({ onApply, T }) {
  ...
  const apply = () => {
    try {
      ...
      if (!parsed.every(n => typeof n === "number" && n >= 0)) throw 0;
      if (parsed.length < 2) throw 0;
      if (parsed.length > 20) { setError("Max 20 values for readability."); return; }
      setError("");
      onApply(parsed);
    } catch {
      setError("Enter 2–20 comma-separated non-negative integers.");
    }
  };
  ...
  return (
    <div style={{
      ...
      <input
        name="container-height" value={val}
